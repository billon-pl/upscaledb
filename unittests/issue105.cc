...
void fill_db( ups_db_t* db, ups_txn_t* txn, const std::vector< uint32_t >& data )
{
    for( uint32_t v : data )
    {
        ...
        // Record has the same value as key!
        // ups_record_t record = ups_make_record( &v, sizeof( v ) );
...
void find_key( ups_db_t* db, ups_txn_t* txn, const std::vector< uint32_t >& query )
{
    for( uint32_t q : query )
    {
        ...
        if ( st == UPS_SUCCESS )
        {
            //const uint32_t rec_data = *reinterpret_cast< uint32_t* >( rec.data );
            //EXPECT_TRUE( q != rec_data , "Key not deleted, A" );
